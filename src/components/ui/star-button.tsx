...
interface StarButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  lightColor?: string;
  /**
   * Fill color of the star-dot rectangle overlay. Set this to match (or nearly
   * match) the button's own background so the holes reveal the orbiting light
   * as sparkles rather than covering the whole button.
   */
  backgroundColor?: string;
  /** Border color of the inner overlay ring */
  borderColor?: string;
  borderWidth?: number;
  /** Class applied to the text span — controls text color/weight */
...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "relative z-[3] overflow-hidden inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50",
        className,
      )}
      {...props}
    >
      ...
      />
      {/* Star dot pattern + border overlay */}
      <div
      ...
      {/* Text */}
      <span className={cn("z-10 relative inline-flex items-center gap-2", textClassName)}>
        {children}
      </span>
    </button>
  );
